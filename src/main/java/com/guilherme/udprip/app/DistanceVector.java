...
public class DistanceVector {
     ...
     */
    public synchronized void applyUpdate(String neighborIp, Map<String, Integer> neighborDistances, int linkWeight) {
        ...
        if (neighborEntry != null) {
            ...
            routingTable.put(neighborIp, new RoutingEntry(neighborIp, linkWeight, neighborIp, neighborIp));
        }
        ...
        for (Map.Entry<String, Integer> entry : neighborDistances.entrySet()) {
            ...
            // Bellman-Ford: Update if we found a better route or this is from the same neighbor we learned from
            RoutingEntry currentEntry = routingTable.get(destination);
            if (currentEntry == null) {
                // New destination
                routingTable.put(destination, new RoutingEntry(destination, distanceThroughNeighbor, neighborIp, neighborIp));
                logger.debug("Added new route to {} via {} with distance {}", destination, neighborIp, distanceThroughNeighbor);
                ...
                logger.debug("Updated route to {} via {} with new distance {}", destination, neighborIp, distanceThroughNeighbor);
                ...
                currentEntry.updateTimestamp();
                logger.debug("Found better route to {} via {} with distance {}", destination, neighborIp, distanceThroughNeighbor);
            }
        }
    }

    /**
     * Get the distances to all known destinations for a specific neighbor,
     * applying split-horizon rule.
     * 
    ...
    public synchronized void invalidateStaleRoutes() {
        ...
        // Find stale entries
        Map<String, RoutingEntry> staleEntries = routingTable.entrySet().stream()
